...
const Business = (props) => {
  ...
  const handleRequestPass = () => {
    console.log(props.route.params.promoid, props.userid, coupondetails, confirmationcode, props.route.params.screen);
    var stringifycoupondetails = JSON.stringify(coupondetails);
    const url = `${apiActiveURL}/promotion/add`;
    let ApiParamForAddCoupon = {
      service_id: props.route.params.screen == 'promotion' ? props.route.params.promoid : props.ServiceId,
      ...
      .then((res) => {
        ...
        if (res.data.code === 200) {
          ...
          if (res.data.hasOwnProperty('data')) {
            props.setFeedback('YourHotel', 'Promotion added successfully', true , '');
            props.navigation.navigate('MyCoupons', { screen: 'Special Promotions' });
          }else{
            props.setFeedback('YourHotel', 'Promotion already exists', true , '');
          }
