...
import { setCarouselCurrentIndexAll, setCarouselTotalIndexAll, setChildCategory, setChildPromotion, setFeedback } from '../actions';
...
const Restaurants = (props) => {
  ...
  const fetchChildCategories = () => {
    ...
    let url = '';
    url = `${apiActiveURL}/categories/${props.CatId}?area=${props.area}&listing_type=7`;
    // if(props.ChildCatId !== 0){
    //   url = `${apiActiveURL}/categories/${props.ChildCatId}?subrub=${props.suburb}&hotel_id=${props.hotelId}&listing_type=7`;  
    // }else{
    ...
    Axios(options)
      .then((res) => {
        if (Object.values(res.data.data).length > 0) {
          ...
          props.setFeedback('YourHotel', 'No Data Found', true, 'Promotions');
          //props.navigation.navigate('PromotionServices');
...
const mapStateToProps = (state) => ({
  ...
  CatId: state.Promotion.id,
  CatName: state.Promotion.name,
  //ChildCatId: state.ChildPromotion.id,
  ChildProName: state.ChildPromotion.name,
