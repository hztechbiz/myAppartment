...
import Tile from '../components/childWOTile';
...
const childCategoriesWhatsOn = (props) => {
  ...
  const fetchChildCategories = () => {
    ...
    if (props.ChildCatId !== 0) {
      url = `${apiActiveURL}/categories/${props.ChildCatId}?subrub=${props.suburb}&area=${props.area}&listing_type=8`;
    } else {
      url = `${apiActiveURL}/categories/${props.CatId}?subrub=${props.suburb}&area=${props.area}&listing_type=8`;
    }
    ...
    Axios(options)
      .then((res) => {
        if (Object.values(res.data.data).length > 0) {
          ...
          props.navigation.navigate('WhatsOnServices');
...
const mapStateToProps = (state) => ({
  ...
  ChildProName: state.ChildWhatsOn.name,
