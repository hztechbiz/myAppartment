...
const TableHead = (props) => {
  ...
  const Items = headings.map((heading, index) => (
    <Text key={index} style={{paddingTop: 8, fontWeight: '700', fontSize: 13}}>
      {heading}
    </Text>
  ));
  return Items;
...
const Business = (props) => {
      ...
      .then((res) => {
        ...
        if (Object.values(res.data.data).length > 0) {
          ...
          if (servicelatitude != undefined) {
            ...
            // console.log(servicelatitude.meta_value, 'latitude');
          } else {
          ...
          if (servicelongitude != undefined) {
            ...
            // console.log(servicelongitude.meta_value, 'longitude');
          } else {
  ...
  const hideOffersModal = (promoid, promotitle) => {
    ...
    props.navigation.navigate('Promotions', {
      ...
      params: {promoid: promoid, promotitle: promotitle, screen: 'promotion'},
    });
    setVisible(false);
  };
