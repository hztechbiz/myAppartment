    ...
    axios(options)
      .then((res) => {
        ...
        if (res.data.code === 200) {
          setCouponServices(res.data.data.sort((a,b) => (a.title > b.title) ? 1 : ((b.title > a.title) ? -1 : 0)));
          setLoader(false);
        } else {
          setCouponServices([])
          console.log('coupons_services', props.userid, res);
      ...
      <View
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <Text style={{
                ...
                marginBottom: 10,
                textAlign: 'center'
              }}>Where the 360 Pass logo appears for a particular restaurant, that means the 360 Pass is available for use at that restaurant. <Image
                    style={{width: 20, height: 20, marginHorizontal: 5}}
                    source={
                      require('../images/wi.png')   
                    }
                  /> means that the 360 Pass is available on weekends as well as mid-week. If <Image
                    style={{width: 20, height: 20, marginHorizontal: 5}}
                    source={
                      require('../images/wid.png')   
                    }
                  /> appears the 360 Pass is not available on weekends - only mid-week.</Text>
