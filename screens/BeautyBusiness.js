...
const BeautyBusiness = (props) => {
            ...
            <>
                <Portal>
                    <Modal
                        ...
                        <Text style={{ textAlign: 'center' }}>{`360 Pass is the IconResort loyalty program entitling you to a discount whenever you visit ${props.ServiceName}.`}</Text>
                        <Button
            ...
            .then((res) => {
                ...
                if (res.data.code === 200) {
                    if (res.data.hasOwnProperty('data')) {
                        addToFavorite(false);
                        props.setFeedback('IconResort', `Pass added to My Favourites. You can use 360 Pass at ${props.ServiceName} and enjoy a discount of ${discount}%.`, true, '');
                        props.navigation.navigate('MyCoupons', { screen: 'My Pass' });
                    } else {
                        props.setFeedback('IconResort', '360 Pass Coupon already exists', true, '');
                    }
                    // setMsgTitle('ClubLocal');
    ...
    const showOffersModal = () => {
        if (promodetails.length > 0) {
            ...
            props.setFeedback('IconResort', `${props.ServiceName} has no Special Offers at the moment`, true, '');
        }
    };
