...
import { setChildCategory, setChildExperience, setChildWhatsOn, setFeedback, setServices } from '../actions';
    ...
    Axios(options)
      .then((res) => {
        if (Object.values(res.data.data).length > 0) {
          ...
          props.setFeedback('YourHotel', 'No Data Found', true , 'Experiences');
          console.log(url, 'categories');
          setLoader2(false);
        }
      })
      .catch((error) => {
        props.setFeedback('YourHotel', 'Something Went Wrong...', true , 'Experiences');
