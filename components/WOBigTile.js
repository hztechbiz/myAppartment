...
import { setPromotionServices, setServices, setWhatsOnServices } from '../actions';
        ...
        onPress={() =>
          {
            ...
            setTimeout(() => {
            setIsPress(false);
            props.setWhatsOnServices(props.services.id , props.services.title);
            navigation.navigate('WhatsOnBusiness');
          }, 0)}}
          >
...
const mapStateToProps = (state) => ({
  ...
  ChildCatName: state.ChildWhatsOn.name,
  token: state.LoginDetails.token,
});

const mapDispatchToProps = (dispatch) => ({
  setWhatsOnServices: (id, name) => {
    const data = {
      id: id,
      name: name,
    };
    dispatch(setWhatsOnServices(data));
  },
});
